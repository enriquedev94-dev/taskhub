...
async def get_tasks(project_id: int, task_service: Annotated[TaskService, Depends(get_task_service)], current_user: Annotated[User, Depends(get_current_user)], pagination: Annotated[PaginationParams, Depends()], search: Annotated[str | None, Query()] = None):
    return task_service.get_tasks(project_id=project_id, owner_id=current_user.id, pagination=pagination, search=search)
